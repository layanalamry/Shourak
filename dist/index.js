...
// server/storage.ts
import { eq, and, desc, inArray, avg, sql as sql2 } from "drizzle-orm";
...
import { pgTable, text, boolean, real, timestamp, uuid, integer } from "drizzle-orm/pg-core";
...
var pendingVerifications = pgTable("pending_verifications", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  full_name: text("full_name").notNull(),
...
var insertPendingVerificationSchema = createInsertSchema(pendingVerifications).omit({ id: true, created_at: true });
var profiles = pgTable("profiles", {
  ...
  show_community_hours: boolean("show_community_hours").default(true),
  avatar_url: text("avatar_url"),
  community_hours: real("community_hours").default(0),
  created_at: timestamp("created_at").defaultNow()
});
var expertTimeslots = pgTable("expert_timeslots", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  expert_id: text("expert_id").notNull(),
  day_of_week: text("day_of_week").notNull(),
  start_time: text("start_time").notNull(),
  end_time: text("end_time").notNull(),
  is_booked: boolean("is_booked").default(false),
  booking_request_id: uuid("booking_request_id"),
  created_at: timestamp("created_at").defaultNow()
});
var bookingRequests = pgTable("booking_requests", {
  ...
  chosen_timeslot_id: uuid("chosen_timeslot_id"),
  duration_minutes: integer("duration_minutes"),
  created_at: timestamp("created_at").defaultNow()
});
var chatMessages = pgTable("chat_messages", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  booking_request_id: uuid("booking_request_id").notNull(),
...
var reviews = pgTable("reviews", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  booking_request_id: uuid("booking_request_id").notNull().unique(),
...
var reports = pgTable("reports", {
  ...
  reported_user_id: text("reported_user_id").notNull(),
...
var insertBookingRequestSchema = createInsertSchema(bookingRequests).omit({ id: true, created_at: true });
var insertTimeslotSchema = createInsertSchema(expertTimeslots).omit({ id: true, created_at: true });
var insertChatMessageSchema = createInsertSchema(chatMessages).omit({ id: true, created_at: true });
var insertReviewSchema = createInsertSchema(reviews).omit({ id: true, created_at: true });
var insertReportSchema = createInsertSchema(reports).omit({ id: true, created_at: true });
...
var DatabaseStorage = class {
  async getProfile(userId) {
    const [row] = await db.select().from(profiles).where(eq(profiles.user_id, userId));
    return row;
  }
  async getProfiles(userIds) {
    if (userIds.length === 0) return [];
    return db.select().from(profiles).where(inArray(profiles.user_id, userIds));
  }
  ...
  async updateProfile(userId, data) {
    const [row] = await db.update(profiles).set(data).where(eq(profiles.user_id, userId)).returning();
    return row;
  }
  async getBookingRequests(filter) {
    if (filter.expertId) {
      return db.select().from(bookingRequests).where(eq(bookingRequests.expert_id, filter.expertId)).orderBy(desc(bookingRequests.created_at));
    }
    if (filter.guestId) {
      return db.select().from(bookingRequests).where(eq(bookingRequests.guest_id, filter.guestId)).orderBy(desc(bookingRequests.created_at));
    }
    return [];
  }
  ...
  async updateBookingRequest(id, data) {
    const [row] = await db.update(bookingRequests).set(data).where(eq(bookingRequests.id, id)).returning();
    return row;
  }
  async getCompletedSessionsForExpert(expertId) {
    const completed = await db.select().from(bookingRequests).where(and(eq(bookingRequests.expert_id, expertId), eq(bookingRequests.status, "completed"))).orderBy(desc(bookingRequests.created_at)).limit(20);
    ...
    const [guestProfiles, slots] = await Promise.all([
      db.select({ user_id: profiles.user_id, name: profiles.name }).from(profiles).where(inArray(profiles.user_id, guestIds)),
      slotIds.length > 0 ? db.select({ id: expertTimeslots.id, start_time: expertTimeslots.start_time, end_time: expertTimeslots.end_time }).from(expertTimeslots).where(inArray(expertTimeslots.id, slotIds)) : Promise.resolve([])
    ]);
  ...
  async getTimeslot(id) {
    const [row] = await db.select().from(expertTimeslots).where(eq(expertTimeslots.id, id));
    return row;
  }
  async getTimeslots(expertId, availableOnly = false) {
    if (availableOnly) {
      return db.select().from(expertTimeslots).where(and(eq(expertTimeslots.expert_id, expertId), eq(expertTimeslots.is_booked, false)));
    }
    return db.select().from(expertTimeslots).where(eq(expertTimeslots.expert_id, expertId));
  }
  ...
  async deleteTimeslotsForExpert(expertId) {
    await db.delete(expertTimeslots).where(eq(expertTimeslots.expert_id, expertId));
  }
  async updateTimeslot(id, data) {
    const [row] = await db.update(expertTimeslots).set(data).where(eq(expertTimeslots.id, id)).returning();
    return row;
  }
  async claimTimeslot(slotId, bookingRequestId) {
    const [row] = await db.update(expertTimeslots).set({ is_booked: true, booking_request_id: bookingRequestId }).where(and(eq(expertTimeslots.id, slotId), eq(expertTimeslots.is_booked, false))).returning();
    return row ?? null;
  }
  async releaseTimeslot(slotId, bookingRequestId) {
    const [row] = await db.update(expertTimeslots).set({ is_booked: false, booking_request_id: null }).where(and(eq(expertTimeslots.id, slotId), eq(expertTimeslots.booking_request_id, bookingRequestId))).returning();
    return row ?? null;
  }
  async getChatMessages(bookingRequestId) {
    return db.select().from(chatMessages).where(eq(chatMessages.booking_request_id, bookingRequestId)).orderBy(chatMessages.created_at);
  }
  ...
  async getReviewsForExpert(expertId) {
    return db.select().from(reviews).where(eq(reviews.expert_id, expertId)).orderBy(desc(reviews.created_at));
  }
  async getReviewByBooking(bookingRequestId) {
    const [row] = await db.select().from(reviews).where(eq(reviews.booking_request_id, bookingRequestId));
    return row;
  }
  ...
  async getAverageRatingForExpert(expertId) {
    const [row] = await db.select({ avg: avg(reviews.rating) }).from(reviews).where(eq(reviews.expert_id, expertId));
    const val = row?.avg;
    if (val === null || val === void 0) return null;
    return parseFloat(String(val));
  }
  async incrementCommunityHours(expertId, hours) {
    await db.update(profiles).set({ community_hours: sql2`community_hours + ${hours}` }).where(eq(profiles.user_id, expertId));
  }
  async isAdmin(userId) {
    const [row] = await db.select().from(userRoles).where(and(eq(userRoles.user_id, userId), eq(userRoles.role, "admin")));
    return !!row;
  }
  async getReports(statusFilter) {
    if (statusFilter) {
      return db.select().from(reports).where(eq(reports.status, statusFilter)).orderBy(desc(reports.created_at));
    }
    return db.select().from(reports).orderBy(desc(reports.created_at));
  }
  async getReport(id) {
...
router.post("/auth/signup", async (req, res) => {
  try {
    ...
    const existing = await db.select().from(users).where(eq2(users.email, email.toLowerCase().trim())).limit(1);
    ...
    await db.delete(pendingVerifications).where(eq2(pendingVerifications.email, email.toLowerCase().trim()));
...
router.post("/auth/verify-otp", async (req, res) => {
  try {
    ...
    const [pending] = await db.select().from(pendingVerifications).where(eq2(pendingVerifications.email, email.toLowerCase().trim())).limit(1);
    if (!pending) {
    ...
    if (/* @__PURE__ */ new Date() > new Date(pending.expires_at)) {
      await db.delete(pendingVerifications).where(eq2(pendingVerifications.id, pending.id));
...
router.post("/auth/login", async (req, res) => {
  try {
    ...
    const [user] = await db.select().from(users).where(eq2(users.email, email.toLowerCase().trim())).limit(1);
    if (!user) {
...
router.post("/auth/forgot-password", async (req, res) => {
  try {
    ...
    if (user) {
      ...
      const expires_at = new Date(Date.now() + 10 * 60 * 1e3);
      await db.delete(passwordResetTokens).where(eq2(passwordResetTokens.user_id, user.id));
...
router.post("/auth/reset-password", async (req, res) => {
  try {
    ...
    const [resetRecord] = await db.select().from(passwordResetTokens).where(
      and2(
        eq2(passwordResetTokens.user_id, user.id),
    ...
    await db.update(users).set({ password_hash }).where(eq2(users.id, user.id));
    await db.delete(passwordResetTokens).where(eq2(passwordResetTokens.id, resetRecord.id));
...
async function runMigrations() {
  try {
    ...
    `);
    const usersExists = await db.execute(sql3`
    ...
    for (const file of files) {
      const rows = await db.execute(
        sql3`SELECT 1 FROM _migrations WHERE name = ${file}`
      );
      if (rows.rows.length === 0) {
        ...
        for (const stmt of statements) {
          await db.execute(sql3.raw(stmt));
        }
        await db.execute(sql3`INSERT INTO _migrations (name) VALUES (${file})`);
...
async function runAutoCompleteSessions() {
  try {
    await db.execute(sql3`SELECT auto_complete_sessions()`);
